...
namespace DSAFinalRequirement.Forms.Products
{
    public partial class EditProductForm : Form
    {
        ...
        private bool ValidateInputs()
        {
            ...
            if (string.IsNullOrWhiteSpace(currentImageName) && string.IsNullOrWhiteSpace(selectedImageFile))
            {
            ...
            try
            {
                using (OleDbConnection conn = DatabaseConnection.GetConnection())
                {
                    ...
                    if (reader.Read())
                    {
                        ...
                        cmbCategoryID.SelectedValue = reader["CategoryID"];
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            string imageNameToSave = currentImageName;

            if (!string.IsNullOrEmpty(selectedImageFile))
            {
                imageNameToSave = Guid.NewGuid() + Path.GetExtension(selectedImageFile);

                string projectRoot = Path.GetFullPath(Path.Combine(Application.StartupPath, @"..\.."));
                string savePath = Path.Combine(projectRoot, "Assets", "Images", "ProductImages", imageNameToSave);
            ...
            try
            {
                using (OleDbConnection conn = DatabaseConnection.GetConnection())
                {
                    ...
                    cmd.Parameters.AddWithValue("ProductImage", imageNameToSave);
